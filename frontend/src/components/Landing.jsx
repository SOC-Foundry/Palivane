...
const FAQ = [
  ...
  { q: "Where does our prompt text actually go?",
    a: SELF_HOSTED
      ...
      : "To Palivane, and nowhere beyond it. By default we record the verdict and its metadata and discard the text itself, so what we hold is that a prompt to ChatGPT carried an AWS key, not the prompt. Scoring is deterministic (rules and heuristics, no AI service in the loop) unless you switch on the optional LLM judge, which does send the content it reviews to a model provider — your own account if you supply a key, otherwise ours. Prefer that none of it reaches us at all? The free self-hosted edition runs the same detection on your own infrastructure, and comes through us while the public release is prepared." },
