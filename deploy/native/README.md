...
(with a generated `WARDEN_SECRET_KEY` and a SQLite default), runs migrations, and starts the
...
- **Use Postgres**, not the default SQLite: edit `DATABASE_URL` in `/etc/warden/warden.env`
  ...
  The app enforces a strong `WARDEN_SECRET_KEY` on non-SQLite (JWTs would be forgeable otherwise).
...
- **Config** lives in `/etc/warden/warden.env` (mode 600) — judge keys, gateway upstreams,
  SMTP, posture toggles (`WARDEN_ENCRYPT_FINDINGS`, `WARDEN_STORE_CONTENT`, `GATEWAY_ENFORCE`).
