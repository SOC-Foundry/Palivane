#!/usr/bin/env bash
# Build a self-contained Warden release tarball for native (no-Docker) install.
#
# Run on a BUILD box that has Python + Node. Produces dist/warden-native-<tag>.tar.gz with
...
cp deploy/warden-api.service "$STAGE/warden-api.service"
cp deploy/warden.env.example "$STAGE/warden.env.example"

tar czf "$OUT/warden-native-$TAG.tar.gz" -C "$(dirname "$STAGE")" warden
rm -rf "$(dirname "$STAGE")"
echo "built $OUT/warden-native-$TAG.tar.gz"
echo "Ship it, then on the target:  tar xzf warden-native-$TAG.tar.gz && sudo ./warden/install.sh"
