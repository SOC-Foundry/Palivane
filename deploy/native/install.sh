#!/usr/bin/env bash
# Warden native installer — run Warden directly on a server, NO Docker.
#
...
# default is a local SQLite file). Sets up a system user, a venv, /etc/warden/warden.env,
...
for f in backend static warden-api.service warden.env.example; do
...
install -m 644 "$SRC/warden-api.service" /etc/systemd/system/warden-api.service
systemctl daemon-reload
systemctl enable --now warden-api
sleep 3
systemctl is-active --quiet warden-api && echo "   warden-api is running" || { echo "   service failed — journalctl -u warden-api"; exit 1; }
...
Installed. The API + console are served on the port in warden-api.service (default 8088);
...
Manage: systemctl {status,restart} warden-api  ·  logs: journalctl -u warden-api -f
EOF
