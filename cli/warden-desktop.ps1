<#
warden-desktop.ps1 — govern *desktop* AI apps (Claude/ChatGPT desktop, pinned IDE
assistants) and AI CLIs on Windows. The PowerShell port of `warden-desktop` (bash), so a
Windows endpoint gets the same egress-governance plane with no admin rights and no Python.

It runs a LOCAL mitmproxy on 127.0.0.1:8081 with Warden's addon, trusts its CA in the
...
token that `warden connect` already stored, so no separate sign-in.

    powershell -ExecutionPolicy Bypass -File warden-desktop.ps1 install            # desktop apps + CLIs
    powershell -ExecutionPolicy Bypass -File warden-desktop.ps1 install -CliOnly   # CLIs only (Claude Code, Codex, Gemini)
    powershell -ExecutionPolicy Bypass -File warden-desktop.ps1 uninstall          # remove proxy, CA trust, task, shims
    powershell -ExecutionPolicy Bypass -File warden-desktop.ps1 status
...
Windows-specific notes (deliberate choices, mirrored from the bash version's posture):
  ...
  * The proxy persists as a per-user Scheduled Task (run at logon, hidden via a wscript
    ...
    generated warden-proxy.cmd sets WARDEN_URL/WARDEN_TOKEN/WARDEN_PROXY_ENFORCE/
    WARDEN_PROXY_USER and starts mitmdump.
  * AI CLIs (Claude Code, Codex, Gemini) are Node tools with their own HTTP client — they
    ...
    in %USERPROFILE%\.warden\bin that set HTTPS_PROXY + NODE_EXTRA_CA_CERTS before the
  ...
  * mitmproxy installs as the self-contained Windows binary (bundles its own Python).
    Pin with $env:WARDEN_MITM_VERSION.
  * The endpoint credential scan (`warden-secrets`, secrets at rest) IS stdlib Python, and
    ...
    warden-secrets.cmd; when none is found it's skipped with a message — never a scheduled
...
Config (env): WARDEN_URL, WARDEN_PROXY_PORT (8081), WARDEN_PROXY_ENFORCE (false),
WARDEN_MITM_VERSION, WARDEN_CLI_TOOLS ("claude codex gemini"),
WARDEN_SECRETS_ENGINE (trufflehog|gitleaks; default = built-in patterns).
...
$Port      = if ($env:WARDEN_PROXY_PORT) { [int]$env:WARDEN_PROXY_PORT } else { 8081 }
$HomeDir   = $env:USERPROFILE
$WDir      = Join-Path $HomeDir ".warden"
$Addon     = Join-Path $WDir "warden_addon.py"
...
# install needs no system Python. Pin with WARDEN_MITM_VERSION.
...
$CliShimTools = if ($env:WARDEN_CLI_TOOLS) { $env:WARDEN_CLI_TOOLS -split "\s+" } else { @("claude", "codex", "gemini") }
$TaskName    = "WardenProxy"
$LauncherCmd = Join-Path $WDir "warden-proxy.cmd"
$LauncherVbs = Join-Path $WDir "warden-proxy.vbs"
...
$SecretsTaskName = "WardenCredentialScan"
$SecretsScript   = Join-Path $WDir "warden-secrets"
$SecretsCmd      = Join-Path $ShimDir "warden-secrets.cmd"
$SecretsEngine   = if ($env:WARDEN_SECRETS_ENGINE) { $env:WARDEN_SECRETS_ENGINE } else { "" }
$InetKey     = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings"
$ShimMarker  = "warden-desktop CLI capture shim"

function Log([string]$msg) { Write-Host "[warden-desktop] $msg" -ForegroundColor Cyan }
function Die([string]$msg) { Write-Host "[warden-desktop] $msg" -ForegroundColor Red; exit 1 }

# The per-user token + URL warden-connect wrote into Claude Code's settings
# (~/.claude/settings.json, env block). Same source as the bash read_token.
function Read-WardenSettings {
    $f = Join-Path $HomeDir ".claude\settings.json"
    if (-not (Test-Path -LiteralPath $f -PathType Leaf)) {
        Die "No Warden token found ($f is missing). Run 'warden-connect' first, then re-run this."
    }
    try { $data = Get-Content -LiteralPath $f -Raw | ConvertFrom-Json }
    catch { Die "Could not parse $f — run 'warden-connect' again, then re-run this." }
    $envBlock = $null
    if ($data -and $data.PSObject.Properties["env"]) { $envBlock = $data.env }
    $tok = ""; $url = ""; $user = ""
    if ($envBlock) {
        if ($envBlock.PSObject.Properties["WARDEN_TOKEN"]) { $tok = "$($envBlock.WARDEN_TOKEN)" }
        if (-not $tok -and $envBlock.PSObject.Properties["ANTHROPIC_AUTH_TOKEN"]) { $tok = "$($envBlock.ANTHROPIC_AUTH_TOKEN)" }
        if ($envBlock.PSObject.Properties["WARDEN_URL"]) { $url = "$($envBlock.WARDEN_URL)" }
        if ($envBlock.PSObject.Properties["WARDEN_USER"]) { $user = "$($envBlock.WARDEN_USER)" }
    }
    if (-not $tok) {
        Die "No Warden token in $f. Run 'warden-connect' first (browser sign-in), then re-run this."
    }
    if (-not $user) { $user = "$($env:USERNAME)@$($env:COMPUTERNAME)" }
    return [pscustomobject]@{ Token = $tok; Url = $url; User = $user }
}

# Effective backend URL: explicit env > what warden-connect stored > the public default.
...
function Get-MitmVersion {
    if ($env:WARDEN_MITM_VERSION) { return $env:WARDEN_MITM_VERSION }
    try {
        $rel = Invoke-RestMethod -Uri "https://api.github.com/repos/mitmproxy/mitmproxy/releases/latest" `
            -Headers @{ "User-Agent" = "warden-desktop/1.0" } -TimeoutSec 15 -UseBasicParsing
...
function Install-Mitm {
    ...
    $zip = Join-Path $env:TEMP "warden-mitmproxy-$ver.zip"
    try {
...
function Fetch-Addon([string]$wardenUrl) {
    New-Item -ItemType Directory -Force -Path $WDir | Out-Null
    Log "fetching capture addon..."
    try {
        Invoke-WebRequest -Uri "$wardenUrl/cli/warden_addon.py" -OutFile $Addon -TimeoutSec 60 -UseBasicParsing
    } catch {
        Die "could not download warden_addon.py from $wardenUrl ($($_.Exception.Message))"
    }
}
...
function Untrust-CA {
    ...
    try {
        ...
        if (-not $victims) { Log "no Warden/mitmproxy CA found in the CurrentUser Root store." }
    } finally { $store.Close() }
}
...
function Refresh-WinInet {
    if (-not ("Warden.WinInet" -as [type])) {
        Add-Type -Namespace Warden -Name WinInet -MemberDefinition @'
[DllImport("wininet.dll", SetLastError = true)]
public static extern bool InternetSetOption(IntPtr hInternet, int dwOption, IntPtr lpBuffer, int dwBufferLength);
'@
    }
    [Warden.WinInet]::InternetSetOption([IntPtr]::Zero, 39, [IntPtr]::Zero, 0) | Out-Null  # SETTINGS_CHANGED
    [Warden.WinInet]::InternetSetOption([IntPtr]::Zero, 37, [IntPtr]::Zero, 0) | Out-Null  # REFRESH
}
...
function Write-Launcher([string]$mitmdump, [string]$wardenUrl, [string]$token, [string]$user) {
    # Honor an existing enforce stance, default to monitor — same as the bash units.
    $enforce = if ($env:WARDEN_PROXY_ENFORCE) { $env:WARDEN_PROXY_ENFORCE } else { "false" }
    ...
    $cmd = @(
        "@echo off",
        "rem Warden desktop egress proxy launcher (auto-generated by warden-desktop.ps1;",
        "rem 'warden-desktop.ps1 uninstall' removes it).",
        "set ""WARDEN_URL=$wardenUrl""",
        "set ""WARDEN_TOKEN=$token""",
        "set ""WARDEN_PROXY_ENFORCE=$enforce""",
        "set ""WARDEN_PROXY_USER=$user""",
...
function Start-ProxyTask {
    ...
    try {
        Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings `
            -Description "Warden desktop egress proxy (mitmdump + warden_addon.py)" -Force | Out-Null
    } catch {
...
function Wire-CliCapture {
    ...
    foreach ($name in $CliShimTools) {
        ...
        $body = @(
            "@echo off",
            "rem $ShimMarker (auto-generated; 'warden-desktop.ps1 uninstall' removes it).",
...
function Install-SecretsScan([string]$wardenUrl, [string]$token, [string]$user) {
    $py = Find-Python
    if (-not $py) {
        ...
        Log "  packaged warden-secrets.exe and point the MDM task at it."
        return
    }
    try {
        Invoke-WebRequest -Uri "$wardenUrl/cli/warden-secrets" -OutFile $SecretsScript `
            -TimeoutSec 60 -UseBasicParsing
    } catch {
        Log "credential scan skipped: could not download warden-secrets ($($_.Exception.Message))."
        return
    }
    New-Item -ItemType Directory -Force -Path $ShimDir | Out-Null
    ...
    # the MDM-generated warden-secrets-task.xml) can invoke one stable path.
    $cmd = @(
        "@echo off",
        "rem Warden endpoint credential scan (auto-generated by warden-desktop.ps1;",
        "rem 'warden-desktop.ps1 uninstall' removes it).",
        "set ""WARDEN_URL=$wardenUrl""",
        "set ""WARDEN_TOKEN=$token""",
        "set ""WARDEN_USER=$user""",
    ...
    try {
        Register-ScheduledTask -TaskName $SecretsTaskName -Action $action -Trigger $trigger `
            -Settings $settings -Description "Warden endpoint credential scan (secrets at rest)" `
            -Force | Out-Null
...
function Remove-SecretsScan {
    try { Unregister-ScheduledTask -TaskName $SecretsTaskName -Confirm:$false -ErrorAction SilentlyContinue } catch {}
...
function Invoke-Install {
    $settings  = Read-WardenSettings
    $wardenUrl = Resolve-WardenUrl $settings
    $mitmdump  = Install-Mitm
    Fetch-Addon $wardenUrl
    ...
    $enforce = Write-Launcher $mitmdump $wardenUrl $settings.Token $settings.User
    ...
    Install-SecretsScan $wardenUrl $settings.Token $settings.User
    if ($CliOnly) {
        Log "done. CLIs ($($CliShimTools -join ' ')) now route through Warden. Enforce mode: $enforce."
        Log "Verify: open a NEW terminal and run 'claude'; check findings in the console."
    } else {
        Log "done. Desktop AI apps + CLIs now route through Warden. Enforce mode: $enforce."
...
function Invoke-Status {
    ...
    if ($enable -eq 1 -and $server -eq "127.0.0.1:$Port") {
        ...
        Write-Host "user proxy: enabled -> $server (not Warden's)"
    } else {
