<#
warden-reenroll.ps1 — Windows-native (Python-free) Claude Code apiKeyHelper that
self-enrolls a per-device key. A PowerShell port of the `warden-reenroll` script so a
...
Config (first found wins), shape {"url","enroll_token","device"}:
    $env:WARDEN_URL / $env:WARDEN_ENROLL_TOKEN   (env overrides, device defaults to user@host)
    %USERPROFILE%\.warden\enroll.json            (per-user)
    %PROGRAMDATA%\Warden\enroll.json             (system-wide, written by the device installer)

    warden-reenroll.ps1                   # print a live device key (apiKeyHelper mode)
    warden-reenroll.ps1 -Refresh          # force a fresh enrollment, ignoring the cache
    warden-reenroll.ps1 -Setup URL TOKEN  # write %USERPROFILE%\.warden\enroll.json
#>
[CmdletBinding()]
param(
    [switch]$Refresh,
...
$UserCfg = Join-Path $env:USERPROFILE ".warden\enroll.json"
$SysCfg  = if ($env:PROGRAMDATA) { Join-Path $env:PROGRAMDATA "Warden\enroll.json" } else { "" }
$KeyCache = Join-Path $env:USERPROFILE ".warden\device-key"
$TimeoutSec = 8

function Write-Log([string]$msg) { [Console]::Error.WriteLine("warden-reenroll: $msg") }
...
function Load-Config {
    ...
    $url = ("$url").TrimEnd("/")
    $token = $env:WARDEN_ENROLL_TOKEN
    if (-not $token -and $cfg) { $token = $cfg.enroll_token }
    $device = if ($cfg -and $cfg.device) { $cfg.device } else { Get-DeviceId }
    if (-not $url -or -not $token) {
        Die "no enrollment config (set WARDEN_URL + WARDEN_ENROLL_TOKEN, or write $UserCfg)."
    }
...
function Invoke-Enroll([object]$cfg) {
    ...
    $resp = Invoke-RestMethod -Uri ($cfg.url + "/api/enroll") -Method Post `
        ...
        -Headers @{ "User-Agent" = "warden-reenroll/1.0" } `
...
# --- main ---
if ($Setup) {
    if (-not $Rest -or $Rest.Count -ne 2) { Die "usage: warden-reenroll.ps1 -Setup URL ENROLL_TOKEN" }
