...
function Fetch-Addon([string]$wardenUrl) {
    ...
    try {
        Invoke-WebRequest -Uri "$wardenUrl/cli/palivane_addon.py" -OutFile $Addon -TimeoutSec 60 -UseBasicParsing
    } catch {
        Die "could not download palivane_addon.py from $wardenUrl ($($_.Exception.Message))"
    }
}
...
function Install-SecretsScan([string]$wardenUrl, [string]$token, [string]$user) {
    ...
    try {
        Invoke-WebRequest -Uri "$wardenUrl/cli/palivane-secrets" -OutFile $SecretsScript `
