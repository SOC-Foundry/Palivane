#!/usr/bin/env python3
"""Warden git capture plane — scan changed files for secrets & PII before they reach a repo.
...
It calls the Warden backend's `POST /api/scan/code` (the detection engine; no secrets
logic lives here). Stdlib only — no pip install.

Config (env):
  WARDEN_URL    Warden backend base URL          (or --url)
  WARDEN_TOKEN  a per-tenant API key (ak_…)       (env only, never a CLI flag)

Usage:
  warden_git_scan.py --staged                 # default: staged changes (pre-commit)
  warden_git_scan.py --range origin/main..HEAD  # a diff range (CI)
  warden_git_scan.py path/a.py path/b.env       # explicit files
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Scan changed files for secrets & PII via Warden.")
    ...
    ap.add_argument("--record", action="store_true", help="persist findings in Warden")
    args = ap.parse_args()
    ...
    blocking = False
    print(f"\n  ⚠ Warden found sensitive data in {len(flagged)} file(s):\n")
    for f in flagged:
