"""Endpoint credential scanner (cli/palivane-secrets): detection, masking, privacy, config."""
...
_path = Path(__file__).resolve().parents[2] / "cli" / "palivane-secrets"
_spec = importlib.util.spec_from_loader("palivane_secrets", SourceFileLoader("palivane_secrets", str(_path)))
...
def test_sweep_roots_env_override(monkeypatch):
    monkeypatch.setenv("PALIVANE_SECRETS_ROOTS", "/etc:/opt")
...
def test_read_config_from_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("PALIVANE_TOKEN", "ak_tok")
    cfg = ws.read_config()
...
def test_read_config_falls_back_to_cursor(monkeypatch, tmp_path):
    ...
    (tmp_path / ".cursor" / "palivane.json").write_text(
