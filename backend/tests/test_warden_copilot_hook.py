"""Copilot hook adapter (cli/warden-copilot-hook): shape dispatch, config, verdicts.
...
_path = Path(__file__).resolve().parents[2] / "cli" / "warden-copilot-hook"
_spec = importlib.util.spec_from_loader("warden_copilot_hook", SourceFileLoader("warden_copilot_hook", str(_path)))
...
def test_config_from_copilot_warden_json(monkeypatch, tmp_path):
    ...
    (d / "warden.json").write_text(json.dumps({"url": "https://w.corp.io", "token": "ak_cop"}))
    cfg = hook.read_config()
