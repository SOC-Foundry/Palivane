"""Gemini CLI hook adapter (cli/warden-gemini-hook): event mapping, config, verdicts."""
...
_path = Path(__file__).resolve().parents[2] / "cli" / "warden-gemini-hook"
_spec = importlib.util.spec_from_loader("warden_gemini_hook", SourceFileLoader("warden_gemini_hook", str(_path)))
...
def test_config_from_gemini_warden_json(monkeypatch, tmp_path):
    for v in ("WARDEN_URL", "WARDEN_TOKEN", "WARDEN_ENFORCE"):
        monkeypatch.delenv(v, raising=False)
    monkeypatch.setenv("HOME", str(tmp_path))
    d = tmp_path / ".gemini"
    d.mkdir(parents=True)
    (d / "warden.json").write_text(json.dumps({"url": "https://w.corp.io", "token": "ak_gem"}))
    cfg = hook.read_config()
...
def test_config_reuses_claude_settings_pair(monkeypatch, tmp_path):
    ...
    (d / "settings.json").write_text(json.dumps(
        {"env": {"WARDEN_URL": "https://w.corp.io", "WARDEN_TOKEN": "ak_cc"}}))
    cfg = hook.read_config()
