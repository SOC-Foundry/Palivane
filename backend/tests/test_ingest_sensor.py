...
def test_benign_mcp_not_persisted_by_default(client, raw_client):
    key = _key(client)
    r = raw_client.post("/api/ingest/mcp", json=BENIGN, headers={"X-Palivane-Token": key}).json()
...
def test_dangerous_mcp_is_persisted(client, raw_client):
    key = _key(client)
    r = raw_client.post("/api/ingest/mcp", json=DANGER, headers={"X-Palivane-Token": key}).json()
