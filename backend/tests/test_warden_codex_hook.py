"""Codex CLI hook adapter (cli/warden-codex-hook): event mapping, config, verdicts."""
...
_path = Path(__file__).resolve().parents[2] / "cli" / "warden-codex-hook"
_spec = importlib.util.spec_from_loader("warden_codex_hook", SourceFileLoader("warden_codex_hook", str(_path)))
...
def test_config_from_codex_warden_json(monkeypatch, tmp_path):
    for v in ("WARDEN_URL", "WARDEN_TOKEN", "WARDEN_ENFORCE"):
        monkeypatch.delenv(v, raising=False)
    monkeypatch.setenv("HOME", str(tmp_path))
    d = tmp_path / ".codex"
    d.mkdir(parents=True)
    (d / "warden.json").write_text(json.dumps({"url": "https://w.corp.io", "token": "ak_cdx"}))
    cfg = hook.read_config()
