"""S3 at-rest scanner (cli/palivane-s3-scan): public-exposure detection, object filtering,
...
_path = Path(__file__).resolve().parents[2] / "cli" / "palivane-s3-scan"
_spec = importlib.util.spec_from_loader("palivane_s3_scan", SourceFileLoader("palivane_s3_scan", str(_path)))
...
def _run_main(monkeypatch, s3, argv, *, url="https://w.example.com", token="ak_test",
    ...
    if url is not None:
        monkeypatch.setenv("PALIVANE_URL", url)
    else:
        monkeypatch.delenv("PALIVANE_URL", raising=False)
    if token is not None:
        monkeypatch.setenv("PALIVANE_TOKEN", token)
    else:
        monkeypatch.delenv("PALIVANE_TOKEN", raising=False)
    monkeypatch.setattr(s3s.sys, "argv", ["palivane-s3-scan", *argv])
...
def test_payload_shape_and_filtering(monkeypatch):
    ...
    assert captured["headers"].get("X-palivane-token") == "ak_test"
