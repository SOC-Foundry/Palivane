...
_path = Path(__file__).resolve().parents[2] / "cli" / "palivane-otel"
_spec = importlib.util.spec_from_loader("palivane_otel", SourceFileLoader("palivane_otel", str(_path)))
...
def test_bridge_batch_accepted(client, raw_client):
    ...
    r = raw_client.post("/api/ingest/mcp/batch", json={"items": items}, headers={"X-Palivane-Token": key})
