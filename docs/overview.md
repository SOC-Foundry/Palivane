...
Plus a **git plane**: a stdlib-only Python scanner runs as a pre-commit hook or GitHub
Action, calling `/api/scan/code` to catch secrets/PII in commits.
...
```
1. Dev machine:  ANTHROPIC_BASE_URL=http://warden/v1  claude
...
- **Per-tenant upstream provider keys** — stored encrypted (Fernet); each org bills to
  its own AI account.
- **Per-tenant judge consent** — opt out of the LLM judge for data-residency reasons
  (the judge ships content to a provider).
...
- **Rate limiting** — DB-backed per-minute gateway quota enforced across workers.
...
- **Settings** — org config (name, judge consent, retention, rate limit), upstream keys,
  SSO, and MFA.
- **Audit** — the admin action log.
- **Coverage reconcile** — compares "who used AI" from your IdP/CASB against captured
  findings to surface blind spots.
