...
5. **Endpoint credential hygiene** — **`palivane-secrets`** scans where infostealers look
   ...
   pipe in via **`palivane-import`** → `/api/scan/import`.
...
settings, OpenAI/Gemini gateway routing, Cursor hooks, and a scheduled `palivane-secrets`
...
stdlib sensors run on the device: **`palivane-mcp`** (stdio-MCP wrapper, inline inspection),
**`palivane-hook`** (Claude Code prompts + tool calls, pre-execution), **`palivane-cursor-hook`**
(Cursor prompts + tool calls), **`palivane-codex-hook`** (Codex CLI prompts + tool calls),
**`palivane-gemini-hook`** (Gemini CLI prompts + tool calls), **`palivane-copilot-hook`**
(GitHub Copilot tool calls — deniable — + prompts, across Copilot CLI, VS Code agent
mode, and the cloud coding agent),
**`palivane-posture`** (continuous IDE/MCP drift reporting),
**`palivane-secrets`** (credentials at rest, optionally driving TruffleHog/Gitleaks), and
**`palivane-import`** (pipe CI scanner output in). They report on a separate ingest quota so
...
- **SecretsAtRestDetector** (surface: `secrets`) — credentials found at rest on a device by
  `palivane-secrets` or a third-party scanner (`credential_at_rest`). Private keys / cloud +
...
```
1. git commit → pre-commit hook → palivane_git_scan.py --staged
