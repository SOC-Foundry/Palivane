...
Settings → **User provisioning (SCIM 2.0)** → Generate token. Copy it now, because only its
...
Attributes*, *Deactivate Users*. Okta probes with `userName eq` filters, which are supported;
...
Token = the token. Entra sends path-less PATCH bodies, which are supported. Start provisioning.
