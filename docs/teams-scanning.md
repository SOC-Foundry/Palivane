...
- **Files shared in Teams** live in SharePoint/OneDrive, so pair with the *SharePoint /
...
- A reply to a thread whose root left the sync window is only seen when that root
  changes again. This is stated here because we'd rather you know the boundary than assume
